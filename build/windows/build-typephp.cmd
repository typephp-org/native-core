...
if not exist "%TYPEPHP_HOME%\tpc.exe" (
    echo ERROR: tpc.exe not found under TYPEPHP_HOME=%TYPEPHP_HOME%
    exit /b 2
)
...
if exist "%REPO_ROOT%\build\artifacts\native_core_smoke.exe" del /q "%REPO_ROOT%\build\artifacts\native_core_smoke.exe"

pushd "%TYPEPHP_HOME%"
"%TYPEPHP_HOME%\tpc.exe" "%REPO_ROOT%\build\typephp\core.yml" --no-color
set "BUILD_EXIT=%ERRORLEVEL%"
popd

if not "%BUILD_EXIT%"=="0" exit /b %BUILD_EXIT%
if not exist "%REPO_ROOT%\build\artifacts\native_core_smoke.exe" (
    echo ERROR: TypePHP did not produce native_core_smoke.exe
    exit /b 3
)
